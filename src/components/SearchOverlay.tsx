...
export const SearchOverlay = ({
  ...
  const [recentSearches, setRecentSearches] = useState<string[]>(() => {
    try {
      const stored = localStorage.getItem('athu-recent-searches');
  ...
  const saveRecentSearch = (searchQuery: string) => {
    ...
    try {
      localStorage.setItem('athu-recent-searches', JSON.stringify(updated));
    } catch {
      /* no-op */
    }
  };
  ...
  const clearRecentSearches = () => {
    setRecentSearches([]);
    try {
      localStorage.removeItem('athu-recent-searches');
    } catch {
      /* no-op */
    }
  };
              ...
              <>
                ...
                {recentSearches.map((search, idx) => (
                  ...
                  >
                    ...
                    <Typography sx={{ fontSize: 13, color: 'text.primary' }}>{search}</Typography>
                  </Box>
                ))}
              </>
            )}
