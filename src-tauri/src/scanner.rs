...
fn parse_and_insert(tx: &rusqlite::Transaction, path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tx.execute(
        "INSERT OR REPLACE INTO tracks (title, file_path, duration, track_number, disc_number, album_id, artist_id) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
        rusqlite::params![&title, path_str, duration as i64, track_number, disc_number, album_id, artist_id]
    )?;
